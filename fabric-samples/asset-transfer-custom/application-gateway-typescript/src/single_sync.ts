/**
 * single_sync : Processes a single file, multiple times, once per split ratio
 * Script does WAIT until the upload completes before processing next ratio ->> ordered
 *
 * This node app takes 1 parameter (example usage : npm run single-sync 10)
...
const resultsFileName = 'results_' + getDateTimeString(new Date(Date.now())) + '_single_sync.csv';
...
async function main(): Promise<void> {
    ...
    for (const splitSize of splitSizes) {
        ...
        const cid = await ipfs.addFile(Path.basename(testFilePath), offChainData);
        const offChainEndTime = Date.now()
